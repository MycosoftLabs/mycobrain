...
// AO pins double as â€œindicator lightsâ€ (PWM RGB)
...
// Two fixed â€œslotsâ€ by address
...
// -------------------------
// SuperMorgIO: retro buzzer music kit (original â€œplatformer vibeâ€)
// -------------------------
static int noteHz(const char* n) {
  if (!n) return 0;
...
void setup() {
  ...
  // This is the big â€œserial doesnâ€™t printâ€ fix.
  delay(BOOT_SERIAL_WAIT_MS);

  for (int i = 0; i < 4; i++) pinMode(AIN_PINS[i], INPUT);

  // Setup â€œindicator lightsâ€ on AO pins
  for (int i = 0; i < 3; i++) {
